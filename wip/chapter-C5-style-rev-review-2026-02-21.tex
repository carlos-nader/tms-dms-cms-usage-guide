...
%   Títulos revisados. Prosa e tabelas revisadas.
% - Stopped at §5.2.4. Pending: §5.2.4 → §5.3.
...
\textbf{CMS Down} grants consent for ECM pod transmission. The XMIT switch on the ECM control panel sets the transmission mode (XMIT 1: AUTO Avionics Priority; XMIT 2: AUTO ECM Priority; XMIT 3: Continuous Jam). The ECM Enable light on the miscellaneous panel reflects consent state.

\begin{hotastable}{CMS Actuation with External ECM Pod}\label{tab:C5-S2-S2-S1-ECM-Pod}
ECM Pod & Down & Short & Grant ECM Consent &
\textbf{CMS Down} illuminates the ECM Enable light. The pod transmits in the mode set by the XMIT switch. Transmission continues until \textbf{CMS Right} is pressed or the RF switch is moved from \texttt{NORM}.
& 2.7.4.2.5 & 28 \\
ECM Pod & Right & Short & Remove ECM Consent &
CMS Right removes consent and places the pod in Standby. The ECM Enable light extinguishes. Transmission does not resume until CMS Down is re-issued.
& 2.7.4.2.5 & 28 \\
\end{hotastable}
...
Each short press of \textbf{CMS Left} advances the IDIAS mode: \texttt{STBY} $\rightarrow$ \texttt{AVNC} (Avionics Priority) $\rightarrow$ \texttt{ECM} (ECM Priority) $\rightarrow$ \texttt{AVNC} $\rightarrow$ \texttt{ECM}.
& 2.7.4.1.2 & 28 \\
IDIAS ECM & Right & Short & Remove ECM Consent &
\textbf{CMS Right} sets IDIAS to \texttt{STBY}, removing consent. \texttt{AVNC} or \texttt{ECM} modes are not restored until CMS Left is re-applied.
& 2.7.4.1.2 & 28 \\
\end{hotastable}
...
A single \textbf{CMS Down} press grants consent to both the ALE-47 CMDS (AUTO or SEMI mode) and the external ECM pod. \textit{Both} systems respond to the \textit{same press} with no distinction between the two.
