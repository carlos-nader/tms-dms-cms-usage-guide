...
IDIAS provides three operational modes controlled via \textbf{CMS Left}: Standby (\texttt{STBY}), Avionics Priority (\texttt{AVNC}), and ECM Priority (\texttt{ECM}). Operational modes are available only when the XMTR button is set to \texttt{OPER}. The active mode is reflected on the HUD, FCR, RWR, and TFR pages.
