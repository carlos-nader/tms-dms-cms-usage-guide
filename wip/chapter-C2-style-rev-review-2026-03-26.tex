...
%============================================================================
% File Name: chapter-C2-style-rev-review-2026-03-26.tex
% Issue: #47 #44
...
% - §2.1 complete: titles revised (§2.1.2 → "SOI Symbology"; §2.1.3 → "SOI-Capable Displays")
% - §2.2 complete: intro + §2.2.1 "Primary and Override Master Modes" — all items
%   validated against Dash-34 §2.1.1.2.1, §2.1.7.5.1–5.3
% - §2.2.1 items: NAV, A-A, A-G, DGFT/MSL OVRD — ACM/AIM defaults removed (DTC-dependent)
% - §2.2.2 (Why HOTAS Switches Behave Differently): ELIMINATED — content redistributed
%   into §2.2.1 closing paragraph and §2.3 opener
% - §2.3 complete: opener + §2.3.1 (DMS), §2.3.2 (TMS), §2.3.3 (CMS) — "— Overview"
%   suffix removed; all three subsections follow DMS model; validated against Dash-34 §2.1.5
% - Chapter order confirmed: DMS=C3, TMS=C4, CMS=C5 (\chapref targets updated accordingly)
...
The SOI mechanism manages \textit{where} HOTAS inputs are directed, not \textit{what} those inputs do. The specific function of a HOTAS input --- for example, whether TMS Up designates a target, breaks a track, or cycles through a mode --- depends on delivery modes, current Master Mode, and active sensor format.
...
The availability of SOI designation varies by Master Mode and MFD format.

Master Mode determines HUD eligibility for SOI designation. MFD formats can be designated as SOI in \textit{all Master Modes}. The HUD, however, can be designated as SOI \textit{only in NAV and A-G Master Modes}.
...
Master Mode is the highest-level configuration state of the F-16 avionics suite. It configures cockpit displays, sensor modes, and weapon options, and determines SOI availability and switch functions.
...
HOTAS switch inputs are interpreted within the active master mode configuration. That configuration can be programmed via the Data Transfer Cartridge (DTC) or adjusted manually in-flight (see \secref{sec:C3-S4-S2}). For complete Master Mode details, see \dashref{2.1.1.2.1}.
...
Full coverage of DMS, TMS, and CMS — including all HOTAS tables — is in \chapref{chap:C3}, \chapref{chap:C4}, and \chapref{chap:C5}, respectively.
