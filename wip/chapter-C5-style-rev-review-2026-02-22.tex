...
%
% Narrative Completion: ~65% (§5.1 completo, §5.2.1.1–§5.2.1.3 completos,
%   §5.2.2 completo, §5.2.3 completo, §5.2.4 completo)
...
% - Pending: §5.3 (CMS Block and Variant Notes).
...
\textbf{CMS Down} grants consent for ECM pod transmission. The XMIT switch on the ECM control panel sets the transmission mode (XMIT 1: AUTO Avionics Priority; XMIT 2: AUTO ECM Priority; XMIT 3: Continuous Jam). The ECM Enable light on the miscellaneous panel reflects consent state. Both systems are inhibited when the pilot moves the RF switch from NORM, or when the landing gear handle is down.
...
When ECM consent is active, both the external ECM pod and internal IDIAS are placed in Standby if the RF switch is moved from NORM to QUIET or SILENT. Returning the RF switch to NORM does \textit{not} restore consent. The pilot must re-issue \textbf{CMS Down} to resume ECM transmission.
