...
Perfeito! Então é progresso parcial na S5.

tex
%============================================================================
% WIP FILE METADATA (NOT RENDERED IN PDF)
%============================================================================
% File Name: section-C4-S3-tms-aa-review-2026-02-16.tex
...
%
% Narrative Completion: ~20% (C4:S3 intro complete, C4:S3:S5 intro complete, 5 subsections remaining)
...
% - C4:S3:S5 (IFF): Introduction FINAL ✓ | Subsubsections S1-S5: Draft structure only
...
%
% Changes This Revision (2026-02-16):
...
%
% Known Issues:
% - C4:S3:S5 subsubsections S1-S5: Narrative structure exists, content incomplete
...
%
% Next Steps:
% 1. Populate narrative for C4:S3:S5:S1 (SCAN Interrogation Mode)
% 2. Populate narrative for C4:S3:S5:S2 (LOS Interrogation Mode)
% 3. Develop Table C4-S3-IFF (hotastable format)
% 4. Populate narrative for C4:S3:S5:S4 (Operational Constraints)
% 5. Populate narrative for C4:S3:S5:S5 (AIFF Availability)
% 6. Repeat for remaining subsections S1-S4, S6 and corresponding tables
% 7. Final validation of all cross-references before integration
%
% Integration Notes:
...
% - C4:S3 Introduction + C4:S3:S5 Introduction ready; remaining content in development
...
Identification Friend or Foe (IFF) interrogation is an air-to-air identification function commanded by TMS Left, enabling the pilot to distinguish friendly aircraft from unknowns and bogeys without removing hands from the flight stick. The interrogation mode executed depends on press duration: a short press activates SCAN interrogation for broad area coverage, while a long press activates Line of Sight (LOS) interrogation for precise single-target queries. Both modes display interrogation responses on the Fire Control Radar (FCR) MFD with symbology overlaid on radar contacts, enabling immediate threat discrimination during multi-target engagements without requiring MFD interaction or datalink correlation (see \secref{sec:C4-S3-S5-S1} and \secref{sec:C4-S3-S5-S2} for details).

IFF interrogation is operationally critical in beyond-visual-range (BVR) engagements, where visual identification is impossible and Rules of Engagement (ROE) require positive identification before weapons employment to prevent fratricide. The interrogation process begins when the interrogating aircraft transmits a coded query to the target aircraft, whose transponder validates the query and transmits a response back to the interrogator. The FCR then displays identification symbology based on the response received: a green circle indicates a friendly aircraft that provided a correct response, while a yellow square identifies a bogey or unknown contact that provided an incomplete response or failed to respond.

Not all F-16 variants are equipped with IFF interrogation capability, as the AIFF (Advanced Interrogator Friend or Foe) system is not universally installed across all F-16 blocks and configurations. When available, TMS Left IFF interrogation requires the AIFF system to be powered and configured via the Up Front Controller (UFC) or backup IFF panel controls. Pilots must verify AIFF availability via cockpit panel configuration before flight; see \secref{sec:C4-S3-S5-S5} for AIFF identification procedures and system limitations. For complete IFF system architecture, operational modes, and control panel operation, see \dashref{2.2.4}.
...
% PARÁGRAFO 1 — TMS Left short press behavior (4-5 sentenças)
% - TMS Left short press (<0.6 seconds) activates SCAN interrogation mode
% - SCAN behavior varies: DCPL (decoupled) or CPL (coupled) configuration
% - DCPL mode: AIFF antenna interrogates 8 beam positions sequentially (left→right), full ±60° azimuth/elevation coverage
% - CPL mode: AIFF FOV couples to FCR field-of-regard, activates only beams needed to cover radar scan volume
% - SCAN displays up to 32 target replies + 8 jamming indicators; if >32 respond, AIFF reports closest 32

% PARÁGRAFO 2 — Symbology and response display (3-4 sentenças)
% - Green circle with mode indication (M1/M2/M3/M4) = friendly (correct response)
% - Yellow square with mode indication = bogey/unknown (incomplete or no response)
% - Current interrogation mode displayed adjacent to acquisition cursor during sequence
% - Valid replies displayed for 2 seconds per mode on MFD
% - See \dashref{2.2.4.4.2.2} for complete symbology details

% PARÁGRAFO 3 — CPL/DCPL configuration (3-4 sentenças)
% - SCAN CPL/DCPL configured via DED SCAN INTG page (LIST RCL button)
% - CPL mode also controllable via FCR CNTL page OSB 10 (see \dashref{2.3.1.5.1.6.2})
% - DCPL provides full ±60° coverage independent of FCR field-of-regard
% - CPL saves time by focusing interrogation only within radar scan volume
...
% PARÁGRAFO 1 — TMS Left long press behavior (4-5 sentenças)
% - TMS Left long press (≥0.6 seconds) activates LOS (Line of Sight) interrogation mode
% - AIFF interrogates only target along FCR line of sight (bugged target in SAM/STT, cursor position in RWS)
% - FCR scan center sent to AIFF, which determines two optimal beams for coverage
% - Range filtering applied: from cursor range position to max selected range on MFD
% - No azimuth or elevation filtering (unlike SCAN CPL mode)

% PARÁGRAFO 2 — LOS target sorting and display (3-4 sentenças)
% - LOS mode displays up to 16 target replies (vs 32 in SCAN)
% - If >16 aircraft respond, AIFF subsystem reports only closest 16 to avionics
% - Valid replies displayed for 1.5 seconds per mode on MFD
% - Cursor position not updated during multi-mode interrogation sequence

% PARÁGRAFO 3 — LOS operational use (3-4 sentenças)
% - LOS mode used for precise interrogation of single target without scanning full FOV
% - Can be initiated in any A-A radar mode, including standby or off
% - Operationally useful when pilot wants to confirm identity of specific contact
% - Example: STT lock on unknown → TMS Left long → LOS interrogation → green circle confirms friendly
% - Provides focused interrogation while conserving interrogation bandwidth in dense target environments
...
% PARÁGRAFO ÚNICO — AIFF availability and variant differences (8-10 sentenças)
