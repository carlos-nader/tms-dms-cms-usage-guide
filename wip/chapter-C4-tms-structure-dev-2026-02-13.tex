...
% Status: GitHub Issue #34 - \misc\old\PREAMBLE-IMPROVEMENTS-RECOMMENDATIONS-v1_1.md
%============================================================================
...
\begin{document}
	...
	% Notes:
	% - Only the structure
	% - C4:S3 in new WIP separated file
	% - Needs full review
...
\begin{enumerate}
    \item \textbf{General TMS concept and timing} (\secref{sec:C4-S1})
    \item \textbf{TMS with SA displays} (HSD, FCR non-tracking modes) (\secref{sec:C4-S2})
    \item \textbf{TMS in Air-to-Air} (FCR tracking, ACM, TWS, IFF) (\secref{sec:C4-S3})
    \item \textbf{TMS in Air-to-Ground} (FCR GM/GMT/SEA, TGP, MARK) (\secref{sec:C4-S4})
    \item \textbf{TMS in weapon employment} (AGM-65, HARM, DTOS, IAM-VIS, CCIP/CCRP) (\secref{sec:C4-S5})
    \item \textbf{Block/variant notes} (\secref{sec:C4-S6})
\end{enumerate}
...
Practice timing in BMS Training Mission 18 (BARCAP) for A-A spotlight search, and Mission 25 (DTOS) for A-G HMCS slaving. See \secref{sec:C3-S3} and \secref{sec:C3-S5} for mode-specific tables.
...
When HSD is SOI (typically via DMS Down in A-A or NAV modes), TMS commands 
interact with HSD cursor for datalink target management, steerpoint selection, 
and threat ring display.

\subsubsection*{TMS Up (HSD SOI)}
\label{sec:C4-S2-S1-S1}

\begin{itemize}[nosep]
	\item Cursor over datalink track: Designates track as PDLT (Pre-Designated Lead Track).
	\begin{itemize}[nosep]
		\item PDLT displays as octagon symbol on HSD.
		\item PDLT track data (range, aspect, altitude) appears on HUD if available.
		\item Used for flight coordination: ``My PDLT, your PDLT'' target deconfliction.
	\end{itemize}
	\item Cursor over steerpoint symbol: Selects steerpoint as active.
\end{itemize}

\subsubsection*{TMS Down (HSD SOI)}
\label{sec:C4-S2-S1-S2}

\begin{itemize}[nosep]
	\item Removes PDLT octagon designation.
	\item If cursor over pre-planned threat with threat ring visible, first press removes 
	threat ring; second press removes PDLT.
\end{itemize}

\subsubsection*{TMS Right (HSD SOI)}
\label{sec:C4-S2-S1-S3}

\begin{itemize}[nosep]
	\item Steps PDLT through datalink tracks (bottom-to-top, right-to-left sequence).
	\item Allows rapid cycling through multiple contacts without cursor slew.
\end{itemize}

\subsubsection*{Operational context (A-A)}
\label{sec:C4-S2-S1-S4}

PDLT designation in A-A master mode provides:
\begin{itemize}[nosep]
	\item Target prioritization in dense datalink picture (8+ contacts).
	\item Flight coordination: Lead designates PDLT, wingman takes non-PDLT targets.
	\item Tactical picture management: PDLT on HVT (High Value Target) for kill priority.
	\item AWACS/GCI integration: Controller assigns targets, pilot designates via PDLT.
\end{itemize}

\subsubsection*{Operational context (NAV)}
\label{sec:C4-S2-S1-S5}

HSD in NAV mode used for steerpoint management and navigation SA. TMS Up selects 
steerpoints hands-on without ICP interaction.

\textbf{Reference:} HSD datalink operations in TO 1F-16CM-34-1-1 \S2.4.3.7.2.1. 
Link 16 PPLI in \S2.4.3.6.1.4.

See \tabref{tab:C4-S2-HSD} for detailed HSD TMS actions.
...
See \tabref{tab:C4-S2-FCR-search} for FCR search mode TMS actions.
...
See \tabref{tab:C4-S4-FCR-AG} for FCR A-G TMS actions.
...
See \tabref{tab:C4-S4-TGP} for TGP TMS actions.
...
See \tabref{tab:C4-S4-MARK} for MARK TMS actions.
...
See \tabref{tab:C4-S5-AGM65} for AGM-65 TMS actions.
...
See \tabref{tab:C4-S5-DTOS} for DTOS TMS actions.
...
See \tabref{tab:C4-S5-IAMVIS} for IAM-VIS TMS actions.
...
See \tabref{tab:C4-S5-CCIP-CCRP} for CCIP/CCRP TMS actions.
...
See \tabref{tab:C4-S5-HARM} for HARM TMS actions.
