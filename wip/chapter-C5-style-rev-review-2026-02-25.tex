...
%============================================================================
% File Name: chapter-C5-style-rev-review-2026-02-25.tex
...
With the ECM pod power switch in OPR, \textbf{CMS Down} grants consent for ECM pod transmission and the ECM Enable light on the left miscellaneous panel illuminates.

The XMIT switch on the ECM control panel sets the ECM operation mode: XMIT 1 (Avionics Priority, AFT antenna only), XMIT 2 (ECM Priority, both FWD and AFT antennas), or XMIT 3 (Active Jam, continuous transmission)

When the RF switch is moved out of NORM, all ECM systems are placed in Standby.

\begin{hotastable}{CMS Actuation with External ECM Pod}\label{tab:C5-S2-S2-S1-ECM-Pod}
ECM Pod & Down & Short & Grant ECM Consent &
The pod transmits in the operation mode set by the XMIT switch. Transmission
continues until CMS Right is pressed or the RF switch is moved out of NORM.
& 2.7.4.1.1, 2.7.4.2.5 & 28 \\
ECM Pod & Right & Short & Remove ECM Consent &
ECM consent is removed and the pod is placed in Standby. The ECM Enable light
extinguishes. Transmission does not resume until the pilot presses CMS Down again.
& 2.7.4.1.1, 2.7.4.2.5 & 28 \\
\end{hotastable}
...
With the XMTR switch in OPER, \textbf{CMS Left} cycles the IDIAS ECM operational mode: Standby (\texttt{STBY}), Avionics Priority (\texttt{AVNC}), and ECM Priority (\texttt{ECM}).
...
IDIAS is set to \texttt{STBY} and consent is removed. \texttt{AVNC} or \texttt{ECM} modes are not restored until the pilot presses CMS Left again.
& 2.7.4.1.2 & 28 \\
\end{hotastable}
...
When equipped with an external ECM pod, \textbf{CMS Down} grants consent to \textit{both} CMDS (in AUTO or SEMI mode) and the external ECM pod \textit{simultaneously}.
...
If expendables fall to or below the bingo quantity, the CMDS continues dispensing. A \texttt{LOW} audio message is issued when an expendable reaches the bingo quantity; an \texttt{OUT} audio message is issued when an expendable is depleted. Dispensing does not stop automatically. The pilot must press \textbf{CMS Right} to inhibit AUTO dispensing, or switch to MAN mode to take direct control.

\paragraph{RF Switch Override:}

When ECM consent is active, the ECM system is placed in Standby when the RF switch is moved from NORM to QUIET or SILENT. Returning the RF switch to NORM does \textit{not} restore consent. The pilot must press \textbf{CMS Down} again to resume ECM transmission.
