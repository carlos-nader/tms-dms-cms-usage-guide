...
The F-16's Hands On Throttle And Stick (HOTAS) architecture enables the pilot to execute critical flight, sensor, and weapon functions without removing hands from the throttle and stick grips. Three switches — the Display Management Switch (DMS), Target Management Switch (TMS), and Countermeasures Management Switch (CMS) — form the core of HOTAS display and sensor management in Falcon BMS.

Understanding these switches requires familiarity with two foundational concepts: the Sensor of Interest (SOI), which determines which display receives HOTAS inputs, and master modes, which define the operational context in which HOTAS switches function. This chapter establishes the conceptual framework necessary to understand the detailed switch behavior covered in Chapters~\ref{chap:C3}, \ref{chap:C4}, and \ref{chap:C5}.
...
The Sensor of Interest (SOI) is the architectural mechanism that routes HOTAS cursor slew commands and certain switch inputs to a specific display or sensor. At any moment, only one display can hold SOI designation. This single-point control architecture allows one set of physical switches to manage multiple displays and sensors without requiring the pilot to select different controls for different tasks.
...
The F-16 cockpit contains multiple displays capable of presenting sensor video, tactical information, and weapon data: the Head-Up Display (HUD), Helmet Mounted Cueing System (HMCS), and two Multifunction Displays (MFDs). Each MFD can show various formats such as Fire Control Radar (FCR), Targeting Pod (TGP), Weapons (WPN), HARM Attack Display (HAD), and Horizontal Situation Display (HSD). The pilot interacts with these displays using HOTAS controls, particularly the cursor slew control (CURSOR/ENABLE) and the Target Management Switch (TMS).

The SOI designation determines \textit{which display or sensor} currently receives these HOTAS inputs. When the pilot slews the cursor or presses TMS Up, the command is routed to whichever display is designated as SOI. This routing architecture serves a critical function: it enables a single set of controls to manage multiple sensors without requiring the pilot to physically reach for different switches or buttons depending on which sensor is in use.

The SOI mechanism manages \textit{where} HOTAS inputs are directed, not \textit{what} those inputs do. The specific function of a HOTAS input — for example, whether TMS Up designates a target, breaks a track, or cycles through a mode — depends on the sensor mode, master mode, and weapon selected. The SOI simply ensures that the input reaches the correct display.

This design reflects the HOTAS philosophy: minimize pilot workload by consolidating control inputs through a limited number of switches while preserving the flexibility to interact with any sensor or display as needed.
...
The pilot can immediately identify which display is the current SOI through unambiguous visual indicators:
...
\begin{itemize}
	...
	\item \textbf{NOT SOI indication:} When a display format is capable of being SOI but is not currently designated as such, the text \texttt{NOT SOI} may appear on the format. This text appears only on SOI-capable formats (such as FCR, TGP, WPN, HAD, and HSD), not on formats that are inherently unable to be SOI.
\end{itemize}
