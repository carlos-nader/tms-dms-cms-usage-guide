...
% - Sections 4.1.1, 4.1.2, 4.1.3 complete per briefing-v0.2.0.1 Section 3.1
...
\subsubsection{Example SOI Flow (Overview)}

\label{sec:C4-S1-S3}

A typical SOI flow during an A-G mission might proceed as follows:

\begin{enumerate}
    \item \textbf{NAV mode, enroute:} HUD/HMCS is the SOI. Pilot monitors navigation symbology heads-up.
    \item \textbf{Target area approach:} Pilot presses \textbf{DMS Down} to transfer SOI to the FCR MFD, enabling radar ground mapping and cursor slew via CURSOR/ENABLE.
    \item \textbf{Initial Point (IP) identification:} Pilot presses \textbf{DMS Down} again to transfer SOI to the TGP MFD for visual confirmation of the target area.
    \item \textbf{Target designation:} With TGP as SOI, pilot slews the crosshair over the target and uses TMS Forward to establish a POINT track.
    \item \textbf{Weapon delivery cues:} Pilot presses \textbf{DMS Up} to return SOI to HUD/HMCS, allowing HUD steering and weapon release symbology to dominate the visual field during the attack run.
\end{enumerate}

This sequence illustrates the DMS's role as a \textit{display orchestrator}: it does not perform targeting or weapon release itself, but ensures that the correct sensor or display is active at each phase of the mission.

In the following sections, this chapter details how the DMS interacts with MFDS format selection (Section 4.2) and how SOI selection varies across sensor and weapon contexts in both A-A and A-G operations (Section 4.3). Block-specific and variant notes are provided in Section 4.4.
