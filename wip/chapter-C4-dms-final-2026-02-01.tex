...
% File Name: chapter-C4-dms-review-2026-02-01.tex
...
The DMS switch responds to short presses for all standard SOI designation and MFD format cycling functions. Sustained or long-duration holds have no effect on SOI selection or format page cycling. The sole exception is the HMCS symbology blanking toggle, which requires a DMS Down hold of approximately 0.5 seconds and does not affect standard SOI designation or format cycling operations (see Section~\ref{sec:C4-S3-S3} for details).
...
\begin{longtable}{L{2.5cm} L{6.5cm} L{6.0cm}}
	\caption{SOI Availabity by Master Mode\label{tab:C4-S1-SOI-by-mode}}\\
...
\begin{longtable}{C{4.5cm} C{4.5cm}}
	\caption{Valid and Invalid SOI MFD Formats \label{tab:C4-S1-MFD-format}}\\
...
DMS Up is a unidirectional command to transfer SOI to the HUD. If the HUD is already the current SOI, pressing DMS Up again \textbf{has no effect} --- SOI remains on the HUD. To transition SOI from the HUD back to the MFDs, the pilot must use DMS Down (see Section~\ref{sec:C4-S3}), not repeated DMS Up presses.
...
\end{hotastable}
\label{tab:C4-S3-DMS-Down-Usage}

% CHAPTER 4: SECTION 4.3.3
\subsection{DMS Down Exception States}
\label{sec:C4-S3-S3}

In certain special states and submodes, DMS Down may: (i) be temporarily ineffective, as a direct reflection of DMS Up (see Section~\ref{sec:C4-S2-S3}):
...
\begin{itemize}
	...
	A sustained DMS Down press held for approximately 0.5 seconds toggles the 	Helmet Mounted Cueing System (HMCS) between displaying symbology and blanked state. A second 0.5-second DMS Down hold restores HMCS symbology display. This feature is independent of HUD or cockpit blanking states and overrides all other blanking mechanisms.
\end{itemize}
