...
The tables below cover CMS actuation organized by system: CMDS (Manual, Automatic, and Semi-Automatic modes) and ECM (external pods ALQ-131/ALQ-184 and internal IDIAS).
...
The ALE-47 CMDS operates in three modes selected via the CMDS CCU MODE knob: Manual (MAN), Automatic (AUTO), and Semi-Automatic (SEMI). The single program to be executed in the chosen mode --- Programs 1--4 --- is selected via the PRGM knob on the CMDS panel.
...
On aircraft equipped with an external ECM pod, \textbf{CMS Down} grants consent to \textit{both} the ALE-47 CMDS (in AUTO or SEMI mode) and the external ECM pod.
