...
%============================================================================
% File Name: chapter-C5-style-rev-review-2026-03-06.tex
...
%   pending.
% - Session 2026-02-25 (author review): §5.2.1.2 table L3 (duplicate Function
%   cell removed); §5.2.2 opening rewritten; §5.2.2.1 title updated, prose
%   and table (L1, L2) rewritten; §5.2.2.2 prose and table (L1, L2) rewritten;
%   §5.2.3 title updated to "CMDS and ECM Joint Consent", prose and table
%   revised; §5.2.4 title updated to "Constraints and Edge Cases", all five
%   paragraphs revised (Bingo Quantity Behavior, RF Switch Override, IDIAS vs.
%   External ECM Pod, Ground Safety). See issue #44 for full change log.
%   §5.3 pending.
% - Session 2026-02-26 (author manual rewrite): §5.1 road-map sentence retained as
%   permitted navigation (STYLE-GUIDE §7); §5.2 opening rewritten ("tables
%   below cover..."); §5.2.1.2 prose rewritten (consent construction
%   uniformized with §5.2.1.3); §5.2.1.3 prose reordered to match operational
%   sequence per Dash-34 §2.7.2.1 (threat detection → COUNTER → consent →
%   dispense). §5.2.2–§5.2.4 and §5.3 pending manual humanization.
%   See issue #44 for full change log.
% - Session 2026-03-03 (author review): §5.3 AI-speak removal pass completed.
%   §5.3 opening S2 rewritten (P3); table caption corrected (P3 + redundant
%   subtitle removed); table L1 col3 "XMIT knob" → "XMIT switch" (P3/Dash-34
%   §2.7.4.1.3); table L2 col3 corrected to "XMTR switch" (technical error,
%   Dash-34 §2.7.4.1.3); §5.3.1 title updated; §5.3.1 prose rewritten (filler
%   removed, P3, P4, MODAL); §5.3.1.1 title typography corrected; §5.3.1.1
%   prose rewritten (P3, technical correction "band" → "program", Dash-34
%   §2.7.4.1.1); §5.3.1.1 table caption corrected (P3); §5.3.1.2 prose
%   rewritten (P3); §5.3.1.2 table caption corrected (P3). See issue #44 for
%   full change log. §5.3 humanization pending.
% - Session 2026-03-03 session 2 (author review): §5.2.2 opening approved
%   (FORMAT-ITAL removed); §5.2.2.1 prose rewritten (XMIT modes as itemize,
%   paragraphs reordered per operational sequence, Dash-34 §2.7.1.1 confirms
%   "left miscellaneous panel") and table approved (L1 col5 PASSIVE-MISUSE
%   corrected; L2 col4/col5 COMPOUND split, Opção B retained); §5.2.2.2 prose
%   approved ("operational" → "operation mode", itemize/setas rejected as
%   redundant with table col5, Dash-34 §2.7.4.1.3 validates XMTR switch/OPER)
%   and table approved (L1 col4 SYNONYM corrected; L2 col4 technical correction
%   "Remove ECM Consent" → "Set ECM to Standby" per Dash-34 §2.7.4.1.2; L2
%   col5 COMPOUND split, consent terminology corrected for IDIAS); §5.2.3 prose
%   and table approved (prose subject clarified, \textit{simultaneously} removed;
%   table col5 simplified). See issue #44 for full change log.
% - §5.2.1.4 "Bypass and Standby Modes" added (issue #48, 2026-03-05): intro
%   prose (P1, P2), hotastable for BYP, \paragraph{Standby (STBY):} prose.
%   Cascade updates applied: §5.2.1 opening, §5.2.3 Joint Consent, §5.2.4
%   State Tracking.
% - Session 2026-03-06 (author review): §5.2.4 "Constraints and Edge Cases" —
%   all five paragraphs approved. State Tracking: consent state not cleared on
%   MAN switch; BYP/STBY have no consent state (Dash-34 §2.7.2.1 NOTE).
%   Bingo Quantity Behavior: scoped to AUTO mode only; CMS Right disables
%   automatic dispensing (Dash-34 §2.7.2.1; "AUTO DISABLE" label confirmed).
%   RF Switch Override: ECM placed in Standby when RF switch moves from NORM;
%   consent not restored on return to NORM; pilot must press CMS Down again
%   (Dash-34 §2.7.1.1). IDIAS vs. External ECM Pod: joint consent applies to
%   external ECM pod configurations only; duplicate "in" removed.
%   Ground Safety: ECM pod in Standby on ground; radiation emitted while
%   CMS Down held (Dash-34 §2.7.1.1 WARNING; "radiation area" terminology
%   confirmed). LOW/OUT messages dropped as out of scope for CMS guide.
%   See issue #44 for full change log.
% - PENDING: §5.2.1.2 table L4 col5: `re-issue` → `presses...again` (confirmed
%   P3 violation, correction pending).
% - PENDING: §5.3 humanization rewrite not yet started.
...
For complete CMDS and ECM operation, see \dashrefs{2.7.1 and 2.7.2}, respectively. Diagrams of the side-stick controller and throttle functions are provided in \dashref{2.1.5}. \figref{fig:f16_hotas_cms_location} below shows the CMS location on the F-16 side-stick controller.
...
In BYP, the CMDS program selection is bypassed: \textbf{CMS Up} dispenses exactly one chaff and one flare per press. Program 6 remains available via \textbf{CMS Left}.
...
Program 6 dispenses independently of the PRGM knob position.
& 2.7.2.2 & --- \\
\end{hotastable}

\paragraph{Standby (STBY):}
In STBY, CMDS programs and bingo quantities can be edited via the UFC. This mode inhibits all chaff and flare release: \textit{no} CMS direction produces a dispensing effect. For program editing procedures, see \dashref{2.7.2.1} and \dashref{2.7.2.3}.
...
CMS interaction with CMDS is \textit{uniform} across all F-16 blocks and variants (see \secref{sec:C5-S2-S1}). F-16 blocks and variants with external pod ECM, however, use \textit{different} CMS procedures and ECM control panels (see \secref{sec:C5-S2-S2}):
...
The variants listed in the following tables are sourced from Dash-34 documentation. Not all may be available as selectable aircraft in Falcon BMS 4.38.1.
...
These block and variant configurations use the ECM Pod Control Panel with manual jamming program selection. \textbf{CMS Down} grants consent for ECM transmission. For detailed procedures, see \secref{sec:C5-S2-S2-S1}.
...
These block and variant configurations use the IDIAS ECM control panel. \textbf{CMS Left} cycles the operational mode. For detailed procedures, see \secref{sec:C5-S2-S2-S2}.
