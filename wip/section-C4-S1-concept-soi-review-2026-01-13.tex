...
The functionality of the DMS—SOI selection, MFD format cycling and all associated behaviors is identical across all F-16 blocks and variants available in Falcon BMS 4.38.1. Differences in aircraft avionics do not alter DMS switch usage. For this reason, all DMS procedures in this chapter apply universally to the entire F-16 family.
...
\begin{itemize}
    ...
    \item \textbf{Targeting Pod (TGP)} — A-G or A-A tracking.
    ...
    \item \textbf{Head-Up Display(HUD)} — Navigation and A-G master modes only (not A-A).
\end{itemize}
...
Conversely, air-to-air and associated override modes eliminate HUD as a selectable SOI. This design reflects the fundamental principle that air-to-air engagements must be driven by primary sensor information rather than derived or display-level cues, although HCMS is always an option.
