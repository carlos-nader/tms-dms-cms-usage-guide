# Falcon BMS TMS/DMS/CMS Guide — WIP File Naming Convention v1.0
## Work-in-Progress File Organization and Nomenclature

**Status:** Official Rule Set (v1.3)  
...
All WIP files are organized in two top-level folders:
...
| `{STATUS}` | one of: `dev`, `review`, `approved` | Development stage (defined Section 2.6) | `dev`, `review`, `approved` |
...
| `{STATUS}` | one of: `dev`, `review`, `final`, `approved` | Development stage | `dev`, `review`, `final`, `approved` |
...
| `{STATUS}` | one of: `dev`, `review`, `final`, `approved` | Development stage (visuals use same codes as text) | `dev`, `review`, `final`, `approved` |
...
2026-01-15: section-C3-S2-fcr-crm-modes-dev-2026-01-15.tex
            (AI generated; never touched by human)
...
1. Use **flat WIP folder** — No subfolders; type-prefixed filenames provide structure.
2. **Mandatory fields** — All filenames must include type prefix, chapter/topic identifier, descriptive title, status, and date.
3. **Status is explicit** — Always include one of the defined status codes; no ambiguity.
4. **Dates are ISO 8601** — Ensures sortability and eliminates ambiguity.
5. **Hyphens only** — No dots except file extension, no spaces, no special characters.
6. **Hierarchy in filename** — Sections show `C3-S2-S1` notation; subsection numbers explicit in filename.
7. **Shallow hierarchies preferred** — Use document structure for deeper nesting; filenames stay readable.
8. **Automation is algorithmic** — Status transitions based on events, not subjective judgment.
9. **Human controls approval** — Only humans decide final status; only humans move files to `approved`.
...
**Document Status:** Production-Ready (v1.3)  
**Effective Date:** 07 January 2026  
**Last Updated:** 07 January 2026, 03:04 -03  
**Version:** v1.3