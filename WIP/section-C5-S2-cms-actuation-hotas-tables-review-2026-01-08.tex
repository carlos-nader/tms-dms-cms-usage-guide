...
\fancyhead[L]{Section 5.2: CMS Switch Actuation}
...
% HOTAS table environment (per BRIEFING v0.1.4.0)
...
\title{F-16 Countermeasures Management Switch (CMS)\\\
...
% FORCE SECTION NUMBERING TO START AT 5.2
...
% SUBSUBSECTION 5.2.1.4: CMDS PROGRAM SELECTION

% ============================================================================

\subsubsection{Program Selection \& Manual Execution}

\label{sec:cms_cmds_programs}

Program selection (Programs 1--4) is controlled via the CMDS panel PRGM knob. Program 5 is always accessible via the cockpit slap switch (S key). Program 6 is always accessible via CMS Left. This section clarifies which programs are available in each CMDS mode.

\begin{hotastable}[CMDS Program Selection \& Manual Execution]

A-A/A-G --- CMDS MAN/SEMI/AUTO & Up & Short & Program 1--4 (PRGM knob selection) &

Executes the program currently selected on the CMDS panel PRGM knob (1, 2, 3, or 4). Each press runs the program once. In AUTO, manual CMS Up overrides continuous dispensing and executes the selected program; AUTO resumes on next threat.

& \dashref{2.7.2.2} & TBD \\

A-A/A-G --- CMDS MAN/SEMI/AUTO & Left & Short & Program 6 (Always Available) &

Executes Program 6 without reference to the PRGM knob. Program 6 is typically configured as flare-only (no chaff) for close-range air-to-air or MANPAD defense. Available in all CMDS modes (MAN, SEMI, AUTO).

& \dashref{2.7.2.2} & TBD \\

A-A/A-G --- CMDS MAN/SEMI/AUTO & S Key (Slap Switch) & Short & Program 5 (Always Available) &

Program 5 is activated via the slap switch on the throttle (not CMS). Program 5 is typically configured as a rapid chaff/flare combo for unexpected or unknown threats. Available in all CMDS modes but is not a CMS function (included for reference).
...
% SUBSECTION 5.2.3: CMS CONSENT & CONSTRAINTS

% ============================================================================

\subsection{CMS Consent \& Constraints}

\label{sec:cms_constraints}
...
\begin{hotastable}[CMS Interaction with CMDS and ECM (Consent \& Constraints)]
