...
Displays that are \textit{not} valid SOI include (these pages provide information or control but do not accept sensor-like slew or targeting inputs): SMS (Stores Management Set), DTE (Data Transfer Equipment), TEST, and blank/inactive MFDS formats.

\begin{itemize}
    \item {Stores Management (SMS)};
    \item {Data Transfer Equipment (DTE)};
    \item {TEST};
    \item {blank/inactive MFDS formats}.
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Vertical (Up / Down):} Selects \textit{which display} is SOI.
        \begin{itemize}
            ...
            \item \textbf{DMS Down:} Cycles SOI between MFDs, or from HUD to an MFD, detailed in Section~\ref{sec:C4-S3}.
        \end{itemize}
    \item \textbf{Horizontal (Left / Right):} Steps through MFDS format pages on the left or right MFD, independently of which display is the SOI (detailed in Section~\ref{sec:C4-S4}).
        \begin{itemize}
...
\paragraph*{A-G Visual Modes (VIS) --- CCIP, DTOS, AGM-65 VIS, IAM-VIS}
