...
When the pilot presses \textbf{DMS Up}, the system attempts to designate the HUD as SOI. If the master mode allows HUD as SOI (as detailed in the following subsections), the action succeeds immediately:
...
\item Any HOTAS inputs that are tied to the SOI (such as CURSOR/ENABLE for symbology positioning, or TMS for target/waypoint designation) now apply to the HUD and its symbology. The specific effect of each control varies by operational context (see sections below for details).
...
When HUD is SOI in NAV, the pilot can control navigation-specific functions via HOTAS inputs directly on the HUD display (use of DED MARK page on \dashref{2.2.5.3}, for instance).

\subparagraph*{A-G Visual Modes (VIS) --- CCIP, DTOS, AGM-65 VIS, IAM-VIS and Related Submodes:}

In visual delivery modes, targets are visually identified and designated by the pilot. The HUD is the \textbf{primary command interface} for all target designation and rejection functions. DMS Up is not merely effective---it is operationally critical. In particular, if the SOI has drifted to another display (e.g., TGP or WPN), the pilot must use DMS Up to restore HUD SOI before visual cueing can be properly controlled.

When the pilot enters an A-G VIS mode (e.g., CCIP and AGM-65 VIS), the avionics system \textit{automatically configures HUD as SOI}. If SOI is later transferred to another display (e.g., TGP or WPN), visual cueing must first be restored to the HUD using DMS Up, as described above.

With HUD as SOI in A-G VIS, the pilot has full command of:

\begin{itemize}

\item \textbf{Visual target designation:} In CCIP the HUD displays a pipper (computed impact point or bullet track line). The pilot maneuvers the aircraft to place the pipper on the intended impact point and commands weapon release with the WPN REL button. With HUD as SOI, CURSOR/ENABLE may be used (when AGR mechanics allow) to refine the impact solution by adjusting the visual reference point on the HUD.

\item \textbf{AGM-65 Maverick VIS target designation:} In AGM-65 VIS, the HUD displays a target designator (TD) box that slews the Maverick's seeker. The pilot uses CURSOR/ENABLE to position the TD box over the target, then TMS Up to lock the seeker. Target rejection (TMS Down) via HUD symbology requires HUD to be SOI, so that TMS commands are routed to the visual TD box rather than to an MFD format.
...
This restriction, however, applies exclusively to the SOI routing architecture (which displays receive HOTAS inputs) and does not eliminate the HUD or HMCS functional capability to acquire, track, or cue targets. For a more complete discussion of HUD/HMCS capabilities in A-A master modes and their relationship to SOI routing, refer to Section~\ref{sec:C4-S1-S3}.

\subsubsection{DMS Up Usage Table}

The Table below summarizes DMS Up behavior across A-A and A-G Master Modes.

In brief, the architectural design is clear: DMS Up is available exactly where the HUD functions as a \textbf{command interface} (NAV and A-G visual modes) and is unavailable where the HUD functions as a \textbf{passive display} (A-A, DGFT, MSL OVRD). This is not an arbitrary constraint; it reflects a deliberate separation between sensor-truth-based operations (air-to-air, where HUD is passive) and display-cueing-based operations (air-to-ground visual and navigation, where HUD is a command interface). Understanding this distinction is fundamental to competent HOTAS management across the entire operational envelope. 
...
\item \textbf{MARK/OFLY Submode:} In the MARK/OFLY submode (a specialized target acquisition context documented in \dashref{2.1.1.2.3}), the SOI cannot be designated at all, and DMS commands are ignored. This exception is rare in normal operations.
...
\item \textbf{MARK/OFLY Submode:} In the MARK/OFLY submode (a specialized target acquisition context documented in \dashref{2.1.1.2.3}), the SOI cannot be designated and DMS commands have no effect. This exception is rare in normal operations.
