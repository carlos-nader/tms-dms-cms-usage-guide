...
% HOTAS table environment (Briefing, Section 6)
...
% File Name: section-C4-S2-dms-up-down-dev-2026-01-13.tex
% WIP Naming Convention: v1.4
% Target Chapter: C4 (DMS — Display Management Switch)
% Target Section: S2 (DMS Up/Down: SOI Management)
% Target Subsection: S2.1 (DMS Up), S2.2 (DMS Down), S2.3 (Placeholder for Master Mode Behavior Table)
...
% Narrative Completion: 95% (S2.1 and S2.2 content complete; S2.3 is placeholder)
% Table Fill Status: 20% (small example tables only; full Master Mode Behavior Table deferred to separate WIP file)

% Notes:
% - Content for 4.2.1 and 4.2.2 complete per DMS-Chapter-Sync-v1.0 Phase 1 roadmap
% - Section 4.2.3 (Master Mode Behavior Table) is PLACEHOLDER ONLY — full table will be in separate WIP file
...
% - Cross-references to Section 4.1 (Concept and SOI) and forward-reference to Section 4.3 (DMS Left/Right) and 4.4 (Integration with TMS)
...
\subsection{DMS Up: HUD/HMCS Designation as SOI}

\label{sec:C4-S2-S1}

% Status: Narrative 95% | Tables 0%
% TODO: None — content complete pending human review
...
In three master modes, DMS Up is fully operative and HUD/HMCS SOI is a valid and often preferred operational state.
...
\subparagraph*{A-G Preplanned Modes (PRE)}

\hl{In preplanned air-to-ground delivery modes, targets are designated pre-mission via steerpoints or pre-entered coordinates. The HUD displays computed delivery solutions (release points, steering cues, altitude/range advisories). DMS Up is \textbf{fully effective} in all A-G PRE submodes.}

When HUD is SOI in A-G PRE, the pilot controls:

\begin{itemize}

\item \textbf{Delivery symbology visualization:} The HUD shows the Continuously Computed Release Point (CCRP) pipper or Dive Toss (DTOS) cues, indicating where ordnance will impact if released at that moment. CURSOR/ENABLE inputs allow fine-tuning of the aircraft's approach to align the pipper with the target.

\item \textbf{Altitude and range awareness:} The HUD displays time-to-go, descent rate cues, and range to target. These cues inform the pilot whether the solution is achievable or requires maneuver adjustment.

\item \textbf{Loft angle and ballistic cueing (DTOS):} In loft delivery modes, the HUD displays the required pitch attitude for a successful toss delivery. TMS inputs may command loft angle adjustments or solution acceptance.

\end{itemize}

\hl{DMS Up is operationally optional in A-G PRE modes (the pilot can use WPN or FCR pages on MFDs), but it is \textbf{highly valuable} for maintaining eyes-out flying during the final approach and delivery phase. Many experienced pilots make HUD their primary SOI immediately before weapons release.} \dashref{4.2.2.2.1}
...
With HUD as SOI in A-G VIS, the pilot has full command of, for instance:
...
In A-A master modes---and also DGFT and MSL OVRD---the avionics architecture \textit{prohibits} HUD from being designated as SOI and DMS Up has no effect Understanding the rationale for this prohibition is crucial to grasping the overall sensor management philosophy.

\subparagraph*{A-A (Air-to-Air) Master Mode:}

In Air-to-Air master mode, the avionics system enforces a strict constraint: \textbf{the SOI is restricted} to FCR (Fire Control Radar), HSD (Horizontal Situation Display), or TGP (Targeting Pod). The \textbf{HUD cannot be SOI}.

The same logic applies to \textbf{DGFT and MSL OVRD}, for they are also A-A modes and assume the same operational logic.

\paragraph*{Summary: DMS Up Effectiveness by Master Mode}

Table~\ref{tab:C4-S2-DMS-Up-Effectiveness} summarizes DMS Up behavior across all master modes:
...
\caption{DMS Up Effectiveness Across All Master Modes}

\label{tab:C4-S2-DMS-Up-Effectiveness}

\begin{tabularx}{15.6cm}{L{2.2cm} C{1.3cm} L{5.5cm} L{6.0cm}}

\rowcolor{headerblue}

\textbf{\color{white}Master Mode} & \textbf{\color{white}DMS Up} & \textbf{\color{white}HUD SOI Permitted?} & \textbf{\color{white}Rationale} \\

\midrule

NAV (Navigation) & Active & Yes & HUD is primary reference for situational awareness; pilot transitions between HUD and MFDs for navigation cueing. \\

\midrule

A-G (PRE) & Active & Yes & HUD displays computed delivery solutions (CCRP, DTOS, altitude cues); optional but valuable for eyes-out delivery. \\

\midrule

A-G (VIS) & Active & Yes & HUD is \textbf{mandatory SOI}; all visual target designation and rejection occurs via HUD-resident symbology and HOTAS inputs. \\

\midrule

A-A (Air-to-Air) & Inactive & No & SOI restricted to FCR/HSD/TGP. HUD is display-only; all tactical actions (tracking, target management) occur on MFD-resident sensors. \\

\midrule

DGFT (Dogfight) & Inactive & No & SOI restricted to FCR/HSD/TGP. HUD displays passive cueing only (horizon, TD box, missile seeker cues); tactical actions remain on radar/tactical display. \\

\midrule

MSL OVRD (Missile Override) & Inactive & No & SOI restricted to FCR/HSD/TGP. Missile assignment and weapon management occur on WPN/SMS pages; HUD is advisory only. \\
...
When ready to resume visual delivery cueing, the pilot presses \textbf{DMS Down again} (or presses \textbf{DMS Down} again depending on cycle configuration) to return the SOI to the HUD. The asterisk reappears, and the pilot resumes CURSOR-based pipper positioning for the final delivery.
...
% TODO: Full Master Mode Behavior Table will be created in separate WIP file (target: section-C4-S2-3-master-mode-table-dev-YYYY-MM-DD.tex)
...
A detailed table with visual indicators, cross-references to \dashref{2.1.1.2.3}, and operational examples will follow in Section 4.2.3.

\subsubsection{Placeholder Note}

This placeholder section will be expanded into a full reference table in a subsequent WIP iteration. The table will include columns for Master Mode, DMS Up behavior, DMS Down behavior, available SOI displays, and notes on special cases.

For now, readers should refer to Section~\ref{sec:C4-S1} (Concept and Sensor of Interest) for the complete list of valid SOI displays by master mode, and to the scenarios in Sections~\ref{sec:C4-S2-S1-S4} and~\ref{sec:C4-S2-S2-S4} for operational context.
