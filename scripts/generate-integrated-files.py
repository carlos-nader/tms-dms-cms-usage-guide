#!/usr/bin/env python3
"""
Generate INTEGRATED-FILES Report (v3 - DEBUG MODE)
...
def main():
    """Main execution"""
    debug_log("=" * 60)
    debug_log("STARTING INTEGRATED-FILES REPORT GENERATION (v3-DEBUG)")
    debug_log("=" * 60)
    ...
    diff_md = subprocess.run(f"git diff --no-index /dev/null {OUTPUT_MD} 2>&1 | wc -l", 
                            shell=True, capture_output=True, text=True, cwd=REPO_ROOT).stdout.strip()
    diff_json = subprocess.run(f"git diff --no-index /dev/null {OUTPUT_JSON} 2>&1 | wc -l", 
                             shell=True, capture_output=True, text=True, cwd=REPO_ROOT).stdout.strip()
    debug_log(f"Git diff output lines (MD): {diff_md}, (JSON): {diff_json}")
